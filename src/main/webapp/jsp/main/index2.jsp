...
<script>
    BUI.use('common/main',function(){
    	var menuarr = document.getElementById('menuarr').value;
    	var config = [
    	...
    	var config_6 = [
    	                     {id:'6',menu:[{text:'产品信息',
						      	  items:[
						    	         ...
						    	         {id:'7',text:'产品分类管理',href:'<%=basePath %>proGroup/init'},
						    	  ]
						      }]}];
    	var config_9 = [
    	                   {id:'9',menu:[{text:'文章信息',
    	...
    	for(var i=1;i<m.length;i++){
    		if(m[i] == '4'){
    			config =  config.concat(config_4);
    		}else if(m[i] == '6'){
    			config =  config.concat(config_6);
    		}else if(m[i] == '9'){
    			config =  config.concat(config_9);
    		}else if(m[i] == '13'){
    			config =  config.concat(config_13);
    		}else if(m[i] == '15'){
    			config =  config.concat(config_13);
    		}
...
<body>
<input type="hidden" value="${menuarr}" readonly="readonly" id="menuarr">
